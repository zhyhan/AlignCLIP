...
from engine import GeneralMovingAverage, get_dataset, get_text_features, train, evaluate_all, AdaptiveLabelSmoothingLoss
...
def main(args: argparse.Namespace):
    ...
    cmodel.convert_weights(classifier)
    classifier.eval()
    #classifier.load_state_dict(torch.load('/home/zhongyi.han/project/CLIPalign/exps/domain_shift/ImageNet/checkpoints/bma.pth'))#

    # obtain text features
    train_text_features, similarity_matrix = get_text_features(clip_model, template, train_class_names, device, args)


    for test_loader in test_loaders:
        test_loader["text_features"], _ = get_text_features(clip_model, template, test_loader["class_names"], device, args)
    ...
    for epoch in range(args.epochs):
        ...
        # train for one epoch
        train(train_iter, classifier, bma_classifier, train_text_features, optimizer, lr_scheduler, epoch, args, writer, device, loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log", type=str, default='max_pooling',
